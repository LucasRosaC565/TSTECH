...
function Sobre() {
  return (
    <section id="sobre" className="items-center flex justify-center py-14">
      <div className="container">
        <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-10 items-start">
          <h2 className="section-title fluid-display">Sobre<br />Nós</h2>
          <p className="section-subtitle max-w-2xl">
          A TS Tech & Health atua desde 2021 no mercado de produtos médicos, oferecendo soluções inovadoras e de alta qualidade nas áreas de Coluna, Neurocirurgia, Ortopedia e Infiltração. Nosso compromisso é unir tecnologia, ética e excelência no atendimento para garantir segurança, confiança e melhores resultados a pacientes e profissionais da saúde.</p>
        </div>
          ...
          {[
            {
              ...
              badge: "/assets/Alvo.png",
              text:
                "Fomentar a inovação com foco nos melhores resultados a pacientes e profissionais.",
            },
            {
              title: "Visão",
              img: "/assets/img2-sobre.jpg",
              badge: "/assets/Olho.png",
              text:
                "Valorizar o trabalho em equipe garantindo qualidade e segurança.",
            },
            {
              title: "Valores",
              img: "/assets/img3-sobre.jpg",
              badge: "/assets/Maos.png",
              text:
                "Compromisso com a excelência e melhoria contínua em nossas ações.",
            },
          ].map((card) => (
            <div key={card.title} className="relative bg-white rounded-2xl shadow-sm border overflow-hidden">
              ...
              />
              <div className="p-5">
                <h3 className="font-bold text-[28px] leading-7 text-[#16514B] mb-3">
                  {card.title}
                </h3>
                <p className="text-[#646464] fluid-body leading-7">{card.text}</p>
              </div>
            </div>
          ))}
        </div>
...
function Tecnologia() {
  const items = [
    { icon: "/assets/Estrela.png", label: "Produtos de Qualidade" },
    { icon: "/assets/Equipe.png", label: "Equipe Treinada" },
    { icon: "/assets/Certificacao.png", label: "Certificação" },
    { icon: "/assets/Emergencia.png", label: "Emergência" },
  ];
  return (
    <section className="relative justify-center items-center py-18 bg-gray-50">
      <div className="container gap-12 justify-center items-center flex lg:justify-start relative z-10 lg:pr-[740px] xl:pr-[840px] 2xl:pr-[900px]">
        <div className="max-w-[760px] text-center justify-center lg:text-left xl:max-w-[820px]">
          ...
          <h2 className="section-title fluid-display mb-4">
            Tecnologia Avançada para Cirurgias Precisas
          </h2>
          
          <p className="section-subtitle max-w-xl mb-6">
          Especializada em materiais minimamente Invasivas para Cirurgias, a TS Tech & Health oferece mais de 70 produtos de alta qualidade para cirurgias de Coluna, Neurocirurgia, Ortopedia e Infiltração. Com atendimento especializado em todo o Brasil, garantimos soluções inovadoras, seguras e em conformidade com as normas da Anvisa. 
          <strong><br />Nosso compromisso é fornecer excelência no atendimento e estabelecer parcerias duradouras, visando sempre os melhores resultados para profissionais da saúde e seus pacientes.</strong>
          </p>
          <ul className="grid grid-cols-2 sm:grid-cols-2 gap-x-[15vw] gap-3 lg:gap-3 mb-6">
            {items.map((it) => (
              <li key={it.label} className="flex items-center gap-3">
                <Image src={it.icon} alt="" width={24} height={24} />
                <span className="text-[#16514B] text-nowrap fluid-label">{it.label}</span>
              </li>
            ))}
          </ul>
          </div>
...
function Produtos() {
  const cats = [
    ...
    { icon: "/assets/Osso.png", name: "Infiltração" },
  ];
  return (
    <section id="produtos" className=" mt-[60vw] lg:mt-0 py-14">
      <div className="container">
        <h2 className="section-title fluid-display text-center mb-4">Nossa Linha de Produtos</h2>
        <p className="text-[#646464] max-w-3xl mx-auto text-center mb-8">
...
function Artigos() {
  ...
  const [articles, setArticles] = useState<Array<{ slug: string; title: string; excerpt: string; image: string; date: string }>>([]);
      ...
      <div className="container">
        ...
        <div className="relative">
          ...
          >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-6 w-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 19l-7-7 7-7"/></svg>
          </button>
          <div
            ref={scrollerRef}
          ...
          >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="h-6 w-6"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 5l7 7-7 7"/></svg>
          </button>
        </div>
      </div>
    </section>
...
function ProdutosDestaque() {
  const [products, setProducts] = useState<Array<{ slug: string; name: string; image: string; category: string }>>([]);
  ...
  return (
    <section className="py-16">
      <div className="container text-center">
        <h2 className="section-title fluid-display mb-3">Produtos Exclusivos e Condições de Convênio Especializadas</h2>
        <p className="text-[#646464] max-w-2xl mx-auto mb-10">
          Produtos exclusivos e convênios especiais para garantir qualidade e condições diferenciadas para você e seus pacientes.
        </p>
        <div className="relative" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {shown.length > 0 ? (
            shown.map((p) => (
              <a key={`${p.category}/${p.slug}`} href={`/catalogo/${p.category}/${p.slug}`} className="group h-28 sm:h-32 md:h-36 rounded-2xl border border-[#16514b33] bg-white shadow-sm overflow-hidden flex items-center justify-center">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.image} alt={p.name} className="h-full w-full object-contain p-2 group-hover:scale-105 transition-transform" />
              </a>
            ))
          ) : (
            [1,2,3,4].map((i) => (
              <div key={i} className="h-28 sm:h-32 md:h-36 rounded-2xl border border-[#16514b33] bg-white shadow-sm" />
            ))
          )}
          </div>
